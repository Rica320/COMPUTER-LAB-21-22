...
int (kbc_read_i)() {
  ...
  if (st & AUX_MOUSE) {
    ...
    if (mouse_received_count == 3) // TODO: MAGIC NUMBER
    {
...
// TODO: CHANGE THIS THIS TO PROB. kbd.c
void (kbc_ih)(void) {
...
int (kbc_send_mouse_cmd)(uint8_t cmd) {
  ...
  do {
    ...
    // CHECKCall(kbc_read(&resp));
    CHECKCall(util_sys_inb(OUT_BUF, &resp));

    if (resp == ACK) { return EXIT_SUCCESS;}
    else if (resp == NACK) {numTries++;}
    else if (resp == KBC_CHECK_ERROR){return EXIT_FAILURE;}
